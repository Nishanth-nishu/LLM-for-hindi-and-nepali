...
def main() -> int:
    ...
    # The measured chars/token, which closes the loop on the budget stages.
    # gcs_ingest and build_corpus size their budgets in characters using a
    # 4.0 proxy, because neither can run after a tokenizer that does not exist
    # yet. This is the real number; pass it back on the second pass and the
    # budgets land where you asked instead of near where you asked.
    chars_by_class: Counter[str] = Counter()
    for s in per_split.values():
        chars_by_class.update(s["characters_by_provenance"])
    total_chars = sum(chars_by_class.values())
    measured_cpt = total_chars / corpus_tokens if corpus_tokens else 0.0
    cpt_by_class = {k: round(chars_by_class[k] / by_class[k], 3)
                    for k in by_class if by_class[k]}
    ...
    report = {
        ...
        "measured_chars_per_token": {
            "overall": round(measured_cpt, 3),
            "by_provenance": cpt_by_class,
            "use": ("Pass this to --chars-per-token on gcs_ingest and "
                    "build_corpus for a second pass whose budgets are exact."),
        },
        "per_split": per_split,
    }
